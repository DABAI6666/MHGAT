...
class BiGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return Data(x=torch.tensor(data['x'], dtype=torch.float32),
                    ...
                    neighbor_row=torch.LongTensor(data['neighbor_row']),
...
def dfs(now, fa, rootindex, root_i,pr):
    if now == rootindex:  # 如果root是bidroot也不用管，因为有5号点
        ...
        if type_node[fa] == 0 and type_node[now] == 1:  # 如果fa是 根结点，那么更新所属类型，并把新的类型和根相连
            ...
            #add
            type_edge[3][1][0].append(len(type_x[3]) - 1)
            type_edge[3][1][1].append(type_index[now])
            # 父节点和新增加的3互联
...
def edge_matrix(edge_index, node_len, root_index, neighbor_row, neighbor_col,pr):
