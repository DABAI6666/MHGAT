...
def check_result(epoch,test_loader,model,testdroprate,mode):
    ...
    for Batch_data in tqdm_test_loader:
        ...
        for i in type_x[4]:
            len_4.append(len(i))
        # print(Counter(len_3), 'Counter(len_3)', Counter(len_4), 'Counter(len_4)')
    ...
    # print('results:', res)
    return np.mean(temp_val_Acc_all), np.mean(temp_val_F1), np.mean(temp_val_F2),np.mean(temp_val_F3), np.mean(temp_val_F4)


def train_MHGAT(treeDic,x_val, x_test, x_train,traindroprate,testdroprate, lr, weight_decay,patience,n_epochs,batchsize,dataname,iter):
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm_train_loader:
            type_edge,type_x,root_id,parent_id = edge_matrix(Batch_data.edge_index.tolist(),len(Batch_data.batch),Batch_data.rootindex.tolist(),Batch_data.neighbor_row.tolist(),Batch_data.neighbor_col.tolist(),traindroprate)
            ...
            out_labels= model(type_edge,x,edge_index,type_x,root_id,parent_id,Batch_data.batch,Batch_data.rootindex)
